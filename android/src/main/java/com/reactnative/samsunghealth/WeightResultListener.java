...
public class WeightResultListener implements
    HealthResultHolder.ResultListener<ReadResult>
{
    ...
    public WeightResultListener(SamsungHealthModule module, Callback error, Callback success) {
    ...
    public void onResult(ReadResult result) {
        ...
        try {
            ...
            if (c != null) {
                if (c.getCount() > 0) {
                    ...
                    int col;
                    while (c.moveToNext()) {
                        String uuid = c.getString(c.getColumnIndex(HealthConstants.Weight.DEVICE_UUID));
                        ...
                        WritableMap map = Arguments.createMap();
                        col = c.getColumnIndex(HealthConstants.Weight.START_TIME);
                        if (col > -1) {
                            map.putDouble(HealthConstants.Weight.START_TIME, (double) c.getLong(col));
                        }
        ...
        Log.d(REACT_MODULE, "Weight " + "Results");
