...
public class OxygenSaturationResultListener implements
    HealthResultHolder.ResultListener<ReadResult>
{
    ...
    public OxygenSaturationResultListener(SamsungHealthModule module, Callback error, Callback success) {
        ...
        try {
            ...
            if (c != null) {
                if (c.getCount() > 0) {
                    ...
                    int col;
                    while (c.moveToNext()) {
                        String uuid = c.getString(c.getColumnIndex(HealthConstants.OxygenSaturation.DEVICE_UUID));
                        ...
                        WritableMap map = Arguments.createMap();
                        col = c.getColumnIndex(HealthConstants.OxygenSaturation.START_TIME);
                        if (col > -1) {
                            map.putDouble(HealthConstants.OxygenSaturation.START_TIME, (double) c.getLong(col));
                        }

                        col = c.getColumnIndex(HealthConstants.OxygenSaturation.SPO2);
                        if (col > -1) {
                            map.putDouble(HealthConstants.OxygenSaturation.SPO2, (double) c.getLong(col));
                        }
