...
public class BloodPressureResultListener implements
    HealthResultHolder.ResultListener<ReadResult>
{
    ...
    public BloodPressureResultListener(SamsungHealthModule module, Callback error, Callback success) {
    ...
    public void onResult(ReadResult result) {
        ...
        try {
            ...
            if (c != null) {
                if (c.getCount() > 0) {
                    ...
                    int col;
                    while (c.moveToNext()) {
                        String uuid = c.getString(c.getColumnIndex(HealthConstants.BloodPressure.DEVICE_UUID));
                        ...
                        WritableMap map = Arguments.createMap();
                        col = c.getColumnIndex(HealthConstants.BloodPressure.START_TIME);
                        if (col > -1) {
                            map.putDouble(HealthConstants.BloodPressure.START_TIME, (double) c.getLong(col));
                        }

                        col = c.getColumnIndex(HealthConstants.BloodPressure.SYSTOLIC);
                        if (col > -1) {
                            map.putInt(HealthConstants.BloodPressure.SYSTOLIC, c.getInt(col));
                        }

                        col = c.getColumnIndex(HealthConstants.BloodPressure.DIASTOLIC);
                        if (col > -1) {
                            map.putInt(HealthConstants.BloodPressure.DIASTOLIC, c.getInt(col));
                        }

                        col = c.getColumnIndex(HealthConstants.BloodPressure.PULSE);
                        if (col > -1) {
                            map.putInt(HealthConstants.BloodPressure.PULSE, c.getInt(col));
                        }
        ...
        for(Map.Entry<String, WritableArray> entry: devices.entrySet()) {
            ...
            map.putArray("bloodPressure", entry.getValue());
            results.pushMap(map);
        }

        Log.d(REACT_MODULE, "Blood Pressure Results");
