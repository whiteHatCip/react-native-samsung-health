...
public class SleepResultListener implements
    HealthResultHolder.ResultListener<ReadResult>
{
    ...
    public SleepResultListener(SamsungHealthModule module, Callback error, Callback success) {
    ...
    public void onResult(ReadResult result) {
        ...
        try {
            ...
            if (c != null) {
                if (c.getCount() > 0) {
                    ...
                    int col;
                    while (c.moveToNext()) {
                        String uuid = c.getString(c.getColumnIndex(HealthConstants.Sleep.DEVICE_UUID));
                        ...
                        WritableMap map = Arguments.createMap();
                        col = c.getColumnIndex(HealthConstants.Sleep.START_TIME);
                        if (col > -1) {
                            map.putDouble(HealthConstants.Sleep.START_TIME, (double) c.getLong(col));
                        }

                        col = c.getColumnIndex(HealthConstants.Sleep.END_TIME);
                        if (col > -1) {
        ...
        for(Map.Entry<String, WritableArray> entry: devices.entrySet()) {
            ...
            map.putArray("sleep", entry.getValue());
            results.pushMap(map);
        }

        Log.d(REACT_MODULE, "Sleep " +  "Results");
